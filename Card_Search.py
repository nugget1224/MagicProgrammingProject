...
# Defines Card_Search function for use later
def Card_Search():
	...
	def return_card_names(cards_list):
		return list(map(lambda x: x["name"], cards_list))
	...
	card = search_cards(query)
	names = return_card_names(card)
	...
	for x in number_choice_array:
		selected_card_array = selected_card_array + [card[int(x)-1]]
	...
	# Ask user if they need a change to set on a chosen card
	query = input("\nDo you need a specific printing of any card? Y/N \n")
	
	if query == "Y":
		print(selected_cards_array)
		#query = print
